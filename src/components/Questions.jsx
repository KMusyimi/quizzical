...
import { StartGameContext } from './Main';
...
function Questions() {
	...
	function handleChange(e) {
		...
		setFormData((prevFormData) => {
			if (prevFormData.length > 0) {
				const idx = prevFormData
					.map((data) => data.parentId)
					.indexOf(answersObj.parentId);
	...
	function handleClick() {
		...
		if (formData.length < 5) {
			...
			document
				.querySelectorAll('label > input[type=radio]')
			...
			formData.forEach((data) => {
				...
				if (idx < 0) {
					document.querySelector(
						`input[value="${data.selectedAnswer}"]`
	...
	function labelElements(arr, idx, parentId) {
		return arr.map((answer, i) => {
			return (
				<label
					key={`label-${id}-${i}`}
					htmlFor={`${answer.toLowerCase().replace(/\s/g, '-')}-${parentId}`}
				>
					{answer}
					<Input
						id={`${answer.toLowerCase().replace(/\s/g, '-')}-${parentId}`}
						type={'radio'}
						name={`answer-${id}-${idx}`}
						checked={
							formData.length > 0
								? formData.filter((data) => data.selectedAnswer === answer)
										.length > 0
								: false
						}
	...
	const questionsElement = () => {
		return questions.map((data, idx) => {
			...
			const parentId = `choices-${idx + 1}`;
			return (
				<section key={nanoid()} className="question">
					<h1>{question}</h1>
					<div id={parentId} className="label-wrapper">
						{labelElements(answers, idx, parentId)}
					</div>
				</section>
			);
		});
	...
	return (
		<>
			...
			{start && !loading && (
				<Form id="questions-form" className="form">
					{questionsElement()}
				</Form>
			)}
			...
			{start && !loading && (
				<div className="btn-container">
					...
					<Button className="btn game-btn" type="button" onClick={handleClick}>
						Check answers
					</Button>
				</div>
			)}
